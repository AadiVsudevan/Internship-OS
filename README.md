...
A personal career-opportunity engine : discovery, deduplication, explainable ranking, Google Sheets tracking, a weekly application queue, draft answers and interview preparation. No paid model calls.

## Deployment

- Repository: https://github.com/AadiVsudevan/Internship-OS
- Destination: private `GOOGLE_SHEET_ID` Actions secret.
...
- Required secret: `GOOGLE_SERVICE_ACCOUNT_JSON`. Share the destination Sheet with that account as Editor. Set `GOOGLE_SHEET_ID` as a second repository secret.
...
`config/personal.json` controls ranking weights and weekly hours. Identity, interests and evidence live only in the private Sheet’s Profile tab. `config/sheets-sources.json` controls sources; `config/deployment.json` names the public repository only. Secrets and generated previews are excluded from Git.
